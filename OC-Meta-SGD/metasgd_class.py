...
class MetaSGD:
    ...
    def __init__(self, sess, args, seed, n_train_tasks, input_shape):
        ...
        self.X_test = tf.placeholder(
            tf.float32, (None,) + input_shape, name='X_test')
        self.Y_test = tf.placeholder(
            tf.float32, (None, self.n_classes), name='Y_test')
        ...
        if(self.anil):
            ...
            for i in range(len(self.m_vars[-self.n_head_params:])):
                self.lrs.append(tf.Variable(tf.ones_like(
                    self.m_vars[-self.n_head_params:][i]) * self.lr, name='lr_' + str(i)))
        ...
        self.test_loss, self.my_acc, self.my_precision, self.my_recall, self.my_specificity, self.my_f1_score, self.my_auc_pr = self.finetune_test_op(
    ...
    def get_first_updated_weights(self, loss):
        ...
        if(self.anil):
            grads = tf.gradients(loss, self.m_vars[-self.n_head_params:])
        else:
            grads = tf.gradients(loss, self.m_vars)
        ...
        if(self.anil):
            ...
            for i in range(1, self.n_head_params + 1):
                # relu is applied to learning rates to clip them to positive
                # values
                new_weights.append(self.m_vars[-self.n_head_params - 1 + i] - tf.keras.activations.relu(
                    self.lrs[-self.n_head_params - 1 + i]) * grads[-self.n_head_params - 1 + i])
        else:
            if(self.bn):
                w_keys = []
                b_keys = []
                ...
                for i in range(0, int(len(self.layers) / 2) + 1):
                    w_keys.append('w' + str(i + 1))
                    b_keys.append('b' + str(i + 1))
                    if(i != int(len(self.layers) / 2)):
                        bn_gamma_keys.append('bn_gamma' + str(i + 1))
                        bn_beta_keys.append('bn_beta' + str(i + 1))

                # relu is applied to learning rates to clip them to positive
                # values
                new_weights = dict(zip(w_keys, [self.m_vars[i] -
                                                tf.keras.activations.relu(self.lrs[i]) *
                                                grads[i] for i in range(0, len(self.m_vars), 4)]))
                new_biases = dict(zip(b_keys, [self.m_vars[i] -
                                               tf.keras.activations.relu(self.lrs[i]) *
                                               grads[i] for i in range(1, len(self.m_vars), 4)]))
                new_bn_gamma = dict(zip(bn_gamma_keys, [self.m_vars[i] - tf.keras.activations.relu(
                    self.lrs[i]) * grads[i] for i in range(2, len(self.m_vars), 4)]))
                new_bn_beta = dict(zip(bn_beta_keys, [self.m_vars[i] - tf.keras.activations.relu(
                    self.lrs[i]) * grads[i] for i in range(3, len(self.m_vars), 4)]))
                ...
                self.lrs_dict = dict(
                    zip(w_keys, [self.lrs[i] for i in range(0, len(self.m_vars), 4)]))
                self.lrs_dict.update(
                    dict(zip(b_keys, [self.lrs[i] for i in range(1, len(self.m_vars), 4)])))
                self.lrs_dict.update(
                    dict(zip(bn_gamma_keys, [self.lrs[i] for i in range(2, len(self.m_vars), 4)])))
                self.lrs_dict.update(
                    dict(zip(new_bn_beta, [self.lrs[i] for i in range(3, len(self.m_vars), 4)])))
                ...
                new_weights = dict(zip(w_keys, [self.m_vars[i] -
                                                tf.keras.activations.relu(self.lrs[i]) *
                                                grads[i] for i in range(0, len(self.m_vars), 2)]))
                new_biases = dict(zip(b_keys, [self.m_vars[i] -
                                               tf.keras.activations.relu(self.lrs[i]) *
                                               grads[i] for i in range(1, len(self.m_vars), 2)]))

                new_weights.update(new_biases)

                self.lrs_dict = dict(
                    zip(w_keys, [self.lrs[i] for i in range(0, len(self.m_vars), 2)]))
                self.lrs_dict.update(
                    dict(zip(b_keys, [self.lrs[i] for i in range(1, len(self.m_vars), 2)])))
    ...
    def get_further_updated_weights(self, loss, old_weights):
        ...
        """
        if(self.anil):
            grads = tf.gradients(loss, old_weights)
        else:
        ...
        if(self.anil):
            # relu is applied to learning rates to clip them to positive values
            new_weights = [
                old_weights[i] -
                tf.keras.activations.relu(
                    self.lrs[i]) *
                grads[i] for i in range(
                    len(old_weights))]
        else:
    ...
    def new_weights_construct_forward(
        ...
        training : bool
            argument for Batch normalization layers.
        X_train_a : tensor
            when a tensor is passed to the function, its BN stats are used in the BN layers
        ...
        if(self.anil):
            ...
            if(X_train_a is not None and self.bn):
                # compute BN stats of X_train_a for all BN layers
    ...
    def metatrain_task(self, X_train_a, Y_train_a,
        ...
        new_weights = self.get_first_updated_weights(self.train_a_loss)
    ...
    def finetune_test_op(
            ...
            Y_test,
            training):
        finetune_output = self.construct_forward(X_finetune, training=training)
        self.finetune_loss = tf.reduce_mean(self.loss_fct(
            labels=Y_finetune,
            logits=finetune_output))
        weights = self.get_first_updated_weights(self.finetune_loss)

        for i in range(self.num_updates - 1):
            finetune_output = self.new_weights_construct_forward(
                X_finetune, weights, training=training)
            self.finetune_loss = tf.reduce_mean(self.loss_fct(
                labels=Y_finetune,
                logits=finetune_output))
            weights = self.get_further_updated_weights(
                self.finetune_loss, weights)

        if(self.bn):
            self.test_output = self.new_weights_construct_forward(
                X_test, weights, training=training, X_train_a=X_finetune)
        else:
            self.test_output = self.new_weights_construct_forward(
                X_test, weights, training=training)
        test_loss = tf.reduce_mean(self.loss_fct(
            labels=Y_test,
            logits=self.test_output))
        my_acc, my_precision, my_recall, my_specificity, my_f1_score, my_auc_pr = self.compute_metrics(
            self.test_output, Y_test)

        return test_loss, my_acc, my_precision, my_recall, my_specificity, my_f1_score, my_auc_pr
    ...
    def val_op(self, K_X_val, K_Y_val, val_test_X, val_test_Y):
        ...
        val_feed_dict = {
            ...
            self.X_test: val_test_X, self.Y_test: val_test_Y
        ...
        if(self.summary):
            ...
            val_summaries, val_test_loss, acc, precision, recall, specificity, f1_score, auc_pr = self.sess.run(
                [self.merged_val, self.test_loss, self.my_acc, self.my_precision, self.my_recall, self.my_specificity, self.my_f1_score, self.my_auc_pr], feed_dict=val_feed_dict)
        else:
            self.sess.run(tf.local_variables_initializer())
            val_test_loss, acc, precision, recall, specificity, f1_score, auc_pr = self.sess.run(
                [self.test_loss, self.my_acc, self.my_precision, self.my_recall, self.my_specificity, self.my_f1_score, self.my_auc_pr], feed_dict=val_feed_dict)
            val_summaries = None
