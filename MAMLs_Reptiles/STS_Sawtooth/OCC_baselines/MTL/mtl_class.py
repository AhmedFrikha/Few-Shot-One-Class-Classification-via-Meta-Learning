...
class MTL:
    ...
    """
    def __init__(self, sess, args, seed, n_train_tasks):
        ...
        self.val_task_finetune_interval = 1000
        self.val_task_finetune_epochs = 15000
        self.early_stopping_val = 500
        ...
        val_finetune_shared_out = self.construct_forward_shared(self.X_finetune, True)
        val_test_shared_out = self.construct_forward_shared(self.X_finetune, False)

        val_scope = "val_task/"
        with tf.variable_scope(val_scope, reuse=tf.AUTO_REUSE):
            self.val_finetune_output = self.feed_forward_task(val_finetune_shared_out, True, -2)
        ...
        finetune_val_task_vars = tf.global_variables(scope=val_scope)
        ...
        self.val_test_output = self.feed_forward_task(val_test_shared_out, False, -2)
        ...
        finetune_task_vars = tf.global_variables(scope=test_scope)
    ...
    def feed_forward_shared(self, inp, update_batchnorm):
        ...
        for i in range(0, len(self.shared_layers)):
    ...
    def val_op(self, X_val_finetune, Y_val_finetune, val_task_test_X, val_task_test_Y, K, cir, mtl_epoch):
        ...
        Returns
        -------
        min_val_test_loss_epoch : int
            finetuning iteration at which the minimal loss on the test set of the validation task was computed in the current validation episode.
        min_val_test_loss : float
            minimal loss computed on the test set of the validation task in the current validation episode.
