...
class FOMAML:
    """ This class defines the model trained with the FOMAML algorithm.
    ...
    def __init__(self, sess, args, seed, n_train_tasks, input_shape):
        ...
        if(self.bn):
            ...
            # with tf.control_dependencies([finetune_update_op, print_op]):
            with tf.control_dependencies([finetune_update_op]):
                # assigning inner loop stats during metatraining
    ...
    def finetune_op(self, K_X_finetune, K_Y_finetune):
        ...
        self.sess.run(self.val_finetune_update_op, feed_dict_finetune)
