...
def load_MNIST(test_task_idx, val_task_idx):
    ...
    train_tasks_file = open(data_path + 'train_tasks_val_' + str(val_task_idx) + '_test_' + str(test_task_idx) + '.txt', 'rb')
    ...
    mtl_train_tasks = {}
    mtl_train_tasks['X'] = np.concatenate((train_tasks['X_train_inner'], train_tasks['X_train_outer']), axis=1)
    mtl_train_tasks['Y'] = np.concatenate((train_tasks['Y_train_inner'], train_tasks['Y_train_outer']), axis=1)


    return mtl_train_tasks, val_task, test_task_finetune_sets, test_task_test_set
...
def sample_random_train_batch(train_tasks, batch_size):
    ...
    for task_idx in range(len(train_tasks['X'])):
...
def main(args):
    ...
    train_tasks, val_task, test_task_finetune_sets, test_task_test_sets = load_MNIST(args.test_task_idx, args.val_task_idx)
    ...
    model = MTL_OCSVM(sess, args, seed)
    ...
    max_val_f1 = {}
    max_val_f1_mtl_epoch = {}

    for K in K_list:
        for cir in cir_inner_loop_list:
            max_val_f1[str(K)+'_'+str(cir)] =-1
            max_val_f1_mtl_epoch[str(K)+'_'+str(cir)] = -1
    ...
    for epoch in range(args.train_epochs+1):
        ...
        train_loss, tr_summaries = model.train_op(X_train, Y_train, epoch)
            ...
            for K in K_list:
                for cir in cir_inner_loop_list:
                    ...
                    if((K==10 and cir ==0.99) or (K<10 and cir not in [0.5, 1.0])):
                        ...
                        if(val_f1_score > max_val_f1[str(K)+'_'+str(cir)]):
                            print('Epoch : ', epoch, ' model saved for K = ', K, ' val_f1 = ', val_f1_score)
                            model.saver.save(
                                model.sess,
                                model.checkpoint_path +
                                model.summary_dir +
                                "_restore_val_task_test_f1_" + str(K) + '_' + str(cir_val_OCSVM) + "/model.ckpt")
                            max_val_f1[str(K)+'_'+str(cir)] = val_f1_score
                            max_val_f1_mtl_epoch[str(K)+'_'+str(cir)] = epoch
