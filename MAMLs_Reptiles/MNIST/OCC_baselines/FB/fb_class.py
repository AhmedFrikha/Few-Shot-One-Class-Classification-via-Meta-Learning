...
class FB:
    ...
    def __init__(self, sess, args, seed):
        ...
        self.X = tf.placeholder(
            tf.float32, (None, self.input_height, self.input_width), name='X')
        ...
        for layer in self.layers:
            if('bn' in layer.name):
                bn_updates_fb.append(layer.updates[-2])
                bn_updates_fb.append(layer.updates[-1])

        with tf.control_dependencies(bn_updates_fb):
        ...
        for layer in self.layers:
            if('bn' in layer.name):
                bn_updates_oc.append(layer.updates[-2])
                bn_updates_oc.append(layer.updates[-1])
        ...
        with tf.control_dependencies(bn_updates_oc):
