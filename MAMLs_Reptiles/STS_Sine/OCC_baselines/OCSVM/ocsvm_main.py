...
def load_STS(K, cir_train):
    """ load the required STS dataset.
        ...
        cir_train : int
            class-imbalance rate of the training set.

        Returns
        -------
        relevant_test_tasks_finetune_sets : list
            list of different training sets sampled from each of the tasks considered.
    ...
    relevant_test_tasks_finetune_sets = [test_task_finetune_sets[str(K)][str(cir_train)] for test_task_finetune_sets in test_tasks_finetune_sets]
    relevant_test_tasks_test_sets = [test_task_test_sets[str(K)] for test_task_test_sets in test_tasks_test_sets]

    return relevant_test_tasks_finetune_sets, relevant_test_tasks_test_sets 
...
def main(args):
    ...
    test_tasks_finetune_sets, test_tasks_test_sets = load_STS(K, cir)
    ...
    for test_task_index, (finetune_sets, test_set) in enumerate(zip(test_tasks_finetune_sets, test_tasks_test_sets)): 
        ...
        for finetune_set_index, finetune_set in enumerate(finetune_sets):
            ...
            # print(finetune_X.shape)
            test_X = np.reshape(test_set['test_X'], (test_set['test_X'].shape[0], -1))
            test_X = pca.transform(test_X)
            ocsvm = initialize_ocsvm(kernel, nu, gamma)
            
            ocsvm = initialize_ocsvm(kernel, nu, gamma)
            
            ocsvm = train(ocsvm, finetune_X, test_X, np.squeeze(test_set['test_Y']), kernel, nu, GridSearch)
            # print('decision was made for gamma = ', ocsvm.gamma)
            acc, prec, rec, spec, f1_score, auc_roc = predict(ocsvm, test_X, np.squeeze(test_set['test_Y']), kernel)
            print('test_task_index: ', test_task_index, 
                'finetune_set_index: ', finetune_set_index, 
                ' acc ', acc,
                ' prec ', prec,
                ' rec ', rec,
                ' spec ', spec,
