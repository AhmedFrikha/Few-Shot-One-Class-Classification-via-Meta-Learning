...
def generate_miniImageNet_tasks():
    ...
    for image_class_idx, image_class in enumerate(train_set):
        class_images = image_class.get_all_images()
        train_images+= class_images
        train_labels+= [image_class_idx]*len(class_images)

    s = np.arange(train_images.shape[0])
    np.random.shuffle(s)
    train_images, train_labels = train_images[s], train_labels[s]
    
    train_data = {'X': train_images,
                  'Y': train_labels}
...
def main(args):
    ...
    train_data, val_tasks, test_tasks = generate_miniImageNet_tasks()
    ...
    if (not (os.path.exists(base_path))):
        base_path = '/home/USER/Projects' 
    dir_path = base_path + '/MAML/input_data/miniImageNet/FB/'
    if (not (os.path.exists(dir_path))):
        os.mkdir(dir_path)
    
    train_data_file = dir_path + 'miniImageNet_train_data.txt'
