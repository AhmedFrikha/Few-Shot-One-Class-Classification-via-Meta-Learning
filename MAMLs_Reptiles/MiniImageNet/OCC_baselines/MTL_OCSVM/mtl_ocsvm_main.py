...
from mtl_ocsvm_class import MTL_OCSVM
...
def load_MiniImageNet_train_val():
    ...
    return mtl_train_tasks, val_tasks



def load_MiniImageNet_test():
...
def sample_random_val_finetune_data(val_tasks, K, cir, n_val_tasks, val_finetune_normal_indexes_list, val_finetune_anomalous_indexes_list):
    ...
    sampled_val_tasks_indices = random.sample(list(np.arange(len(val_tasks))), n_val_tasks)

    for val_task_idx in sampled_val_tasks_indices:
    ...
    return val_X_sampled_list, val_Y_sampled_list, sampled_val_tasks_indices
...
def main(args):
    ...
    train_tasks, val_tasks = load_MiniImageNet_train_val()
    ...
    model = MTL_OCSVM(sess, args, seed, input_shape, n_train_tasks)
            ...
            for K in K_list:
                for cir in cir_inner_loop_list:
                    ...
                    if((K==10 and cir ==0.99) or (K<10 and cir not in [0.5, 1.0])):
                        ...
                        X_val_finetune_list, Y_val_finetune_list, sampled_val_tasks_indices = sample_random_val_finetune_data(val_tasks, K, cir_val_OCSVM, n_val_tasks, val_finetune_normal_indexes_list, val_finetune_anomalous_indexes_list)
                        val_metrics_list = []
                        for val_task_idx, X_val_finetune, Y_val_finetune in zip(sampled_val_tasks_indices, X_val_finetune_list, Y_val_finetune_list):
    ...
    test_tasks = load_MiniImageNet_test()
    ...
    for K in K_list:
        ...
        for test_task_idx, test_task in enumerate(test_tasks):
            ...
            encoding_test = sess.run(model.val_test_shared_out, feed_dict=test_feed_dict)
