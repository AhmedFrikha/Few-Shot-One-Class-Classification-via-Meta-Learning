            ...
            for K in K_list:
                for cir in cir_inner_loop_list:
                    ...
                    if((K==10 and cir ==0.99) or (K<10 and cir not in [0.5, 1.0])):
                        ...
                        X_val_finetune_list, Y_val_finetune_list, sampled_val_tasks_indices = sample_random_val_finetune_data(val_tasks, K, cir_val_IF, n_val_tasks, val_finetune_normal_indexes_list, val_finetune_anomalous_indexes_list)
                        val_metrics_list = []
    ...
    for K in K_list:
        ...
        for test_task_idx, test_task in enumerate(test_tasks):
            ...
            for fset_idx in range(n_finetune_sets):
                ...
                acc, prec, rec, spec, f1_score, auc_roc = predict(isoForest, encoding_test, np.squeeze(test_task["Y_outer"]))
