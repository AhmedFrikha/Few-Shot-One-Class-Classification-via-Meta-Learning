...
def main(args):
    ...
    model = FB_IF(sess, args, seed, input_shape, n_train_classes)
            ...
            for K in K_list:
                for cir in cir_inner_loop_list:
                    ...
                    if((K==10 and cir ==0.99) or (K<10 and cir not in [0.5, 1.0])):
                        ...
                        X_val_finetune_list, Y_val_finetune_list = sample_random_val_finetune_data(val_tasks, K, cir_val_IF, val_finetune_normal_indexes_list, val_finetune_anomalous_indexes_list)
                        val_metrics_list = []
    ...
    for K in K_list:
        ...
        filename = args.summary_dir + '_K_' + str(K) + '_cir_' + str(int(100*cir_val_IF)) +'.txt'
