...
def load_Omniglot():
    ...
    print('using only 10 training tasks for MTL training')
...
def main(args):
    ...
    for K in K_list:
        ...
        for test_task_idx, test_task in enumerate(test_tasks):
            ...
            ocsvm = train(ocsvm, encoding_finetune, encoding_test, np.squeeze(test_task["Y_outer"]), kernel, nu, GridSearch)
            acc, prec, rec, spec, f1_score, auc_roc = predict(ocsvm, encoding_test, np.squeeze(test_task["Y_outer"]), kernel)
