...
def MetaOptNetHead_OC_SVM_batched(
        query,
        support,
        support_labels,
        n_way,
    ...
    Q_spd = 1.0 * torch.eye(n_support).expand(tasks_per_batch,
    ...
    G = Variable(torch.cat([-1.0 * torch.eye(n_support),
                            torch.eye(n_support)],
                           dim=0).expand(tasks_per_batch,
                                         2 * n_support,
                                         n_support))
    h_task = torch.cat([torch.zeros((n_support, 1)).cuda(
    ), (1 / (nu * n_support)) * torch.ones((n_support, 1)).cuda()], dim=0)
    h = torch.squeeze(
        Variable(
            h_task.expand(
                tasks_per_batch,
...
def MetaOptNetHead_Ridge(
    ...
    # min_{\alpha}  0.5 \sum_m ||w_m(\alpha)||^2 + \sum_i \sum_m e^m_i
    # alpha^m_i
    ...
    support_labels_one_hot = support_labels_one_hot.reshape(
        n_way * tasks_per_batch, n_support)     # (n_way*tasks_per_batch, n_support)

    G = block_kernel_matrix
    e = -2.0 * support_labels_one_hot

    # This is a fake inequlity constraint as qpth does not support QP without
...
def MetaOptNetHead_SVM_He(
    ...
    V = (support_labels * n_way - torch.ones(tasks_per_batch,
                                             n_support, n_way).cuda()) / (n_way - 1)
...
def MetaOptNetHead_SVM_CS(
    ...
    id_matrix_2 = torch.eye(n_support).expand(
        tasks_per_batch, n_support, n_support).cuda()
...
def MetaOptNetHead_SVM_WW(
    ...
    kernel_matrix = computeGramMatrix(
        support, support) + torch.ones(tasks_per_batch, n_support, n_support).cuda()

    id_matrix_0 = torch.eye(n_way).expand(tasks_per_batch, n_way, n_way).cuda()
    block_kernel_matrix = batched_kronecker(id_matrix_0, kernel_matrix)

    kernel_matrix_mask_x = support_labels.reshape(
        tasks_per_batch, n_support, 1).expand(
        tasks_per_batch, n_support, n_support)
    kernel_matrix_mask_y = support_labels.reshape(
        tasks_per_batch, 1, n_support).expand(
        tasks_per_batch, n_support, n_support)
    ...
    block_kernel_matrix -= (2.0 - 1e-4) * (kernel_matrix_mask_second_term * \
    ...
    # (tasks_per_batch, n_way, n_support)
    ...
    # Compute the classification score.
    compatibility = computeGramMatrix(
        support, query) + torch.ones(tasks_per_batch, n_support, n_query).cuda()
    compatibility = compatibility.float()
    compatibility = compatibility.unsqueeze(1).expand(
    ...
    qp_sol = qp_sol.float().unsqueeze(3).expand(
        tasks_per_batch, n_way, n_support, n_query)
    Y_support_reshaped = Y_support.reshape(tasks_per_batch, n_way, n_support)
    Y_support_reshaped = A_i * Y_support_reshaped
    Y_support_reshaped = Y_support_reshaped.unsqueeze(
        3).expand(tasks_per_batch, n_way, n_support, n_query)
...
class ClassificationHead(nn.Module):
    def __init__(
            self,
            base_learner='MetaOptNet',
            enable_scale=True,
            enable_shift=False,
            learn_nu=False):
        super(ClassificationHead, self).__init__()
        if('OC-SVM' in base_learner):
            self.head = MetaOptNetHead_OC_SVM_batched
